...
type CreateIndexRequest struct {
	...
	// Name The name of the index. The maximum length is 45 characters.  It may contain lowercase alphanumeric characters or hyphens,  and must not begin or end with a hyphen.
	Name IndexName `json:"name"`

	// Spec Spec objects describe how the index should be deployed
...
// CreateIndexRequest_Spec Spec objects describe how the index should be deployed
type CreateIndexRequest_Spec struct {
	// Pod Configuration needed to deploy a pod index
...
// IndexModel The IndexModel describes the configuration and deployment status of a Pinecone index.
...
// IndexName The name of the index. The maximum length is 45 characters.  It may contain lowercase alphanumeric characters or hyphens,  and must not begin or end with a hyphen.
type IndexName = string

// PodSpec Configuration needed to deploy a pod index
type PodSpec struct {
	// Environment The environment where the index is hosted.
	Environment string `json:"environment"`

	// MetadataConfig Configuration for the behavior of Pinecone's internal metadata index. By default, all metadata is indexed; when `metadata_config` is present, only specified metadata fields are indexed. These configurations are only valid for use with pod indexes.
...
type ServerlessSpec struct {
	...
	// Region The region where you would like your index to be created. Different cloud providers have different regions available.  See AwsRegions and GcpRegions for a list of available options.
...
// N429QuotaExceeded The response shape used for all error responses
type N429QuotaExceeded = ErrorResponse
...
type CreateCollectionResponse struct {
	...
	JSON405      *N405MethodNotSupportedForServerless
...
func ParseCreateCollectionResponse(rsp *http.Response) (*CreateCollectionResponse, error) {
	...
	case strings.Contains(rsp.Header.Get("Content-Type"), "json") && rsp.StatusCode == 405:
		var dest N405MethodNotSupportedForServerless
		if err := json.Unmarshal(bodyBytes, &dest); err != nil {
			return nil, err
		}
